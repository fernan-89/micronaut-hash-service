 ...
 * This immutable command record serves as the formal request payload for temporary deactivation (suspension)
 * workflows. It enforces strict declarative constraints via Jakarta Bean Validation and defensive runtime
 * sanitization to guarantee data integrity before entering the core application layer.
 *
 * <p><b>Contractual Obligations:</b>
 * <ul>
 * <li><b>Immutability:</b> Implemented as a Java record to guarantee thread-safe propagation across reactive pipelines.</li>
 ...
 * </ul>
 *
 * @param hashId   The universally unique identifier (UUID) of the HashToken to be deactivated. Must not be null.
 * @param executor The principal identifier of the user or system authorizing this action. Must not be blank.
 * @param reason   The business justification provided for the operational suspension. Must be between 5 and 500 characters.
 *
 * @author ThinkLab
 * @version 1.0.0
 * @since 1.0
...
public record DeactivateHashCommand(
    ...
    public Compact constructor {
        ...
        if (executor.isBlank() || reason.length() < 5) {
            log.error("[ACTION: DEACTIVATE_HASH_VALIDATION] [ID: {}] [EXECUTOR: {}] - CRITICAL: Pipeline aborted due to malformed command input.", hashId, executor);
